import { Resource } from '@/types/resources'

export const TECHNOLOGIES_RESOURCES: Resource[] = [
  {
    name: 'NodeJs',
    children: [
      {
        name: 'YouTube',
        isFolder: true,
        children: [
          {
            name: 'Node.js Tutorial',
            href: 'https://youtu.be/-MTSQjw5DrM?si=yhIxxMtHYKjxPnsb',
            type: 'youtube',
          },
          {
            name: 'Node.js Tutorial for Beginners',
            href: 'https://youtube.com/playlist?list=PLC3y8-rFHvwh8shCMHFA5kWxD9PaPwxaY&si=Lw_0Cu5C9Huj9sNW',
            type: 'youtube',
          },
        ],
      },
      {
        name: 'Udemy',
        isFolder: true,
        children: [
          {
            name: 'Node.js, Express, MongoDB & More',
            href: 'https://www.udemy.com/course/nodejs-express-mongodb-bootcamp/',
            type: 'article',
          },
          {
            name: 'Node.js Internals and Architecture',
            href: 'https://www.udemy.com/course/nodejs-internals-and-architecture/?couponCode=KEEPLEARNING',
            type: 'article',
          },
        ],
      },
      {
        name: 'FrontEndMasters',
        isFolder: true,
        children: [
          {
            name: 'Introduction to Node.js, v3',
            href: 'https://frontendmasters.com/courses/node-js-v3/',
            type: 'article',
          },
          {
            name: 'API Design in Node.js, v4',
            href: 'https://frontendmasters.com/courses/api-design-nodejs-v4/',
            type: 'article',
          },
        ],
      },
    ],
  },
  {
    name: 'NextJs',
    children: [
      {
        name: 'YouTube',
        isFolder: true,
        children: [
          {
            name: 'Next.js Tutorial',
            href: 'https://www.youtube.com/watch?v=Sklc_fQBmcs',
            type: 'youtube',
          },
          {
            name: 'Next.js Full Course',
            href: 'https://www.youtube.com/watch?v=Zq5fmkH0T78',
            type: 'youtube',
          },
          {
            name: 'Next.js App Router Tutorial',
            href: 'https://youtu.be/k7o9R6eaSes?si=LqDnhs3epoDAMYQk',
            type: 'youtube',
          },
        ],
      },
      {
        name: 'FrontEndMasters',
        isFolder: true,
        children: [
          {
            name: 'Next.js, v4',
            href: 'https://frontendmasters.com/courses/next-js-v4/',
            type: 'article',
          },
          {
            name: 'Intermediate Next.js',
            href: 'https://frontendmasters.com/courses/intermediate-next-js/',
            type: 'article',
          },
        ],
      },
      {
        name: 'Articles',
        isFolder: true,
        children: [
          {
            name: 'Next.js Dashboard App',
            href: 'https://nextjs.org/learn/dashboard-app/getting-started',
            type: 'article',
          },
        ],
      },
    ],
  },
  {
    name: 'ReactJs',
    children: [
      {
        name: 'YouTube',
        isFolder: true,
        children: [
          {
            name: 'React Tutorial for Beginners',
            href: 'https://youtu.be/TPACABQTHvM?si=8eHfoKPLkV4MI96Y',
            type: 'youtube',
          },
          {
            name: 'React Tutorial Playlist',
            href: 'https://www.youtube.com/playlist?list=PLC3y8-rFHvwgg3vaYJgHGnModB54rxOk3',
            type: 'youtube',
          },
        ],
      },
      {
        name: 'FrontEndMasters',
        isFolder: true,
        children: [
          {
            name: 'Complete Intro to React, v9',
            href: 'https://frontendmasters.com/courses/complete-react-v9/',
            type: 'article',
          },
        ],
      },
      {
        name: 'Udemy',
        isFolder: true,
        children: [
          {
            name: 'The Ultimate React Course',
            href: 'https://www.udemy.com/course/the-ultimate-react-course/',
            type: 'article',
          },
          {
            name: 'React - The Complete Guide',
            href: 'https://www.udemy.com/course/react-the-complete-guide-incl-redux/?couponCode=KEEPLEARNING',
            type: 'article',
          },
        ],
      },
      {
        name: 'Articles',
        isFolder: true,
        children: [
          {
            name: 'Tao of React',
            href: 'https://alexkondov.com/tao-of-react/',
            type: 'article',
          },
          {
            name: 'React Foundations',
            href: 'https://nextjs.org/learn/react-foundations/what-is-react-and-nextjs',
            type: 'article',
          },
        ],
      },
    ],
  },
  {
    name: 'JavaScript',
    children: [
      {
        name: 'YouTube',
        isFolder: true,
        children: [
          {
            name: 'JavaScript Tutorial for Beginners',
            href: 'https://www.youtube.com/watch?v=W6NZfCO5SIk&pp=ygUTamF2YXNjcmlwdCB0dXRvcmlhbA%3D%3D',
            type: 'youtube',
          },
          {
            name: 'JavaScript Tutorial Playlist',
            href: 'https://www.youtube.com/playlist?list=PLC3y8-rFHvwhI0V5mE9Vu6Nm-nap8EcjV',
            type: 'youtube',
          },
          {
            name: 'JavaScript Course',
            href: 'https://www.youtube.com/watch?v=GM6dQBmc-Xg&list=PLDoPjvoNmBAx3kiplQR_oeDqLDBUDYwVv',
            type: 'youtube',
          },
        ],
      },
      {
        name: 'Udemy',
        isFolder: true,
        children: [
          {
            name: 'The Complete JavaScript Course',
            href: 'https://www.udemy.com/course/the-complete-javascript-course/',
            type: 'article',
          },
        ],
      },
      {
        name: 'FrontEndMasters',
        isFolder: true,
        children: [
          {
            name: 'Deep JavaScript Foundations, v3',
            href: 'https://frontendmasters.com/courses/deep-javascript-v3/',
            type: 'article',
          },
          {
            name: 'JavaScript: The Hard Parts, v2',
            href: 'https://frontendmasters.com/courses/javascript-hard-parts-v2/',
            type: 'article',
          },
        ],
      },
    ],
  },
  {
    name: 'TypeScript',
    children: [
      {
        name: 'YouTube',
        isFolder: true,
        children: [
          {
            name: 'TypeScript Tutorial for Beginners',
            href: 'https://www.youtube.com/watch?v=d56mG7DezGs',
            type: 'youtube',
          },
          {
            name: 'TypeScript Full Course',
            href: 'https://youtu.be/zQnBQ4tB3ZA?si=byskn3nX1p1Ksz5g',
            type: 'youtube',
          },
          {
            name: 'TypeScript Crash Course',
            href: 'https://youtu.be/EcCTIExsqmI?si=06DjwCL0qYc2pAbj',
            type: 'youtube',
          },
        ],
      },
      {
        name: 'FrontEndMasters',
        isFolder: true,
        children: [
          {
            name: 'TypeScript Fundamentals, v4',
            href: 'https://frontendmasters.com/courses/typescript-v4/',
            type: 'article',
          },
        ],
      },
      {
        name: 'Articles',
        isFolder: true,
        children: [
          {
            name: 'Total TypeScript Tutorials',
            href: 'https://www.totaltypescript.com/tutorials',
            type: 'article',
          },
          {
            name: 'TypeHero',
            href: 'https://typehero.dev/explore',
            type: 'article',
          },
        ],
      },
    ],
  },
  {
    name: 'GraphQL',
    children: [
      {
        name: 'YouTube',
        isFolder: true,
        children: [
          {
            name: 'GraphQL Full Course',
            href: 'https://youtu.be/eIQh02xuVw4?si=WF7LoeBEQeqNfY9h',
            type: 'youtube',
          },
          {
            name: 'GraphQL Crash Course',
            href: 'https://youtu.be/ZQL7tL2S0oQ?si=1A5v1rO4BckiYZqp',
            type: 'youtube',
          },
          {
            name: 'GraphQL Tutorial',
            href: 'https://youtu.be/5199E50O7SI?si=fIE_2XJOkVIN0_Zc',
            type: 'youtube',
          },
        ],
      },
      {
        name: 'FrontEndMasters',
        isFolder: true,
        children: [
          {
            name: 'Advanced GraphQL, v2',
            href: 'https://frontendmasters.com/courses/advanced-graphql-v2/',
            type: 'article',
          },
          {
            name: 'Client-Side GraphQL in React',
            href: 'https://frontendmasters.com/courses/client-graphql-react-v2/',
            type: 'article',
          },
        ],
      },
      {
        name: 'Articles',
        isFolder: true,
        children: [
          {
            name: 'GraphQL Tutorial',
            href: 'https://www.educative.io/blog/graphql-tutorial',
            type: 'article',
          },
          {
            name: 'TutorialsPoint GraphQL',
            href: 'https://www.tutorialspoint.com/graphql/index.htm',
            type: 'article',
          },
          {
            name: 'GeeksforGeeks GraphQL Tutorial',
            href: 'https://www.geeksforgeeks.org/graphql-tutorial/',
            type: 'article',
          },
        ],
      },
    ],
  },
  {
    name: 'Docker',
    children: [
      {
        name: 'YouTube',
        isFolder: true,
        children: [
          {
            name: 'Docker Tutorial for Beginners',
            href: 'https://youtu.be/rIrNIzy6U_g?si=20N-TldYScp9seGo',
            type: 'youtube',
          },
          {
            name: 'Docker Crash Course',
            href: 'https://youtu.be/GFgJkfScVNU?si=jnqqpvlpL4uX-byC',
            type: 'youtube',
          },
          {
            name: 'Docker Tutorial',
            href: 'https://youtu.be/pTFZFxd4hOI?si=0OKWqf2X5_ygAOqO',
            type: 'youtube',
          },
          {
            name: 'Docker Full Course',
            href: 'https://youtu.be/eGz9DS-aIeY?si=CP6nYYbppNKTlzVJ',
            type: 'youtube',
          },
        ],
      },
    ],
  },
  {
    name: 'C',
    children: [
      {
        name: 'YouTube',
        isFolder: true,
        children: [
          {
            name: 'C Programming Tutorials',
            href: 'https://www.youtube.com/playlist?list=PLA1FTfKBAEX4hblYoH6mnq0zsie2w6Wif',
            type: 'youtube',
          },
        ],
      },
    ],
  },
  {
    name: 'C++',
    children: [
      {
        name: 'YouTube',
        isFolder: true,
        children: [
          {
            name: 'Mosh | C++ Tutorial for Beginners',
            href: 'https://www.youtube.com/watch?v=ZzaPdXTrSb8&pp=ygUMQysrIFR1dG9yaWFs',
            type: 'youtube',
          },
          {
            name: 'C++ Full Course',
            href: 'https://www.youtube.com/watch?v=-TkoO8Z07hI&pp=ygUMQysrIFR1dG9yaWFs',
            type: 'youtube',
          },
          {
            name: 'FreeCodeCamp | C++ Tutorial for Beginners',
            href: 'https://www.youtube.com/watch?v=vLnPwxZdW4Y&pp=ygUMQysrIFR1dG9yaWFs',
            type: 'youtube',
          },
        ],
      },
    ],
  },
  {
    name: 'DB',
    children: [
      {
        name: 'YouTube',
        isFolder: true,
        children: [
          {
            name: 'AMR Elhelw | Relational Database Internals (English)',
            href: 'https://www.youtube.com/playlist?list=PLE8kQVoC67PywFpq0VXxGFbStvtskNVkW',
            type: 'youtube',
          },
          {
            name: 'AMR Elhelw | Relational Database Internals (Arabic)',
            href: 'https://www.youtube.com/playlist?list=PLE8kQVoC67PzGwMMsSk3C8MvfAqcYjusF',
            type: 'youtube',
          },
          {
            name: '(Not a tutorial) but a good overview',
            href: 'https://www.youtube.com/watch?v=3gVBjTMS8FE&t=618s&pp=ygUJZGF0YSBiYXNl',
            type: 'youtube',
          },
        ],
      },
    ],
  },
  {
    name: 'Git & GitHub',
    children: [
      {
        name: 'YouTube',
        isFolder: true,
        children: [
          {
            name: 'Elzero | Learn Git and Github',
            href: 'https://www.youtube.com/playlist?list=PLDoPjvoNmBAw4eOj58MZPakHjaO3frVMF',
            type: 'youtube',
          },
          {
            name: 'Ghareeb | Github in one hour',
            href: 'https://www.youtube.com/watch?v=fDkR0TDR9dI&t=2226s&pp=ygUPZ2l0aHViIHR1dG9yaWFs',
            type: 'youtube',
          },
          {
            name: 'Big Data | Github in depth',
            href: 'https://www.youtube.com/watch?v=Q6G-J54vgKc&t=9965s&pp=ygUPZ2l0aHViINi02K3YqNi3',
            type: 'youtube',
          },
        ],
      },
    ],
  },
]
