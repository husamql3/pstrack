...
class RetroEffectImpl extends Effect {
	...
	get colorNum(): number {
		return this.uniforms.get("colorNum")?.value ?? 0
	}
	...
	get pixelSize(): number {
		return this.uniforms.get("pixelSize")?.value ?? 0
	}
}
...
function DitheredWaves({
	...
	const getMaterialUniforms = useCallback(() => {
		const material = mesh.current?.material
		return material instanceof THREE.ShaderMaterial ? material.uniforms : null
	}, [])

	useEffect(() => {
		const u = getMaterialUniforms()
		if (!u) return
