...
import { injectUser, isAuthenticated } from "@/server/lib/session"
		...
		app
			...
			.post(
				...
				async ({ body, user }) => {
					...
					if (result.error === "FULL")
						return status(409, { error: "This group is full." })
			...
			.patch(
				...
				async ({ params, body, user }) => {
					...
					if (result.error === "FORBIDDEN") {
						return status(403, {
							error: "You must be an admin to process join requests.",
						})
					}
					if (result.error === "NOT_FOUND")
					...
					if (result.error === "FULL")
						return status(409, { error: "The group is now full." })
			...
			.post(
				"/:id/invite",
				async ({ params, body, user }) => {
					const result = await groupsDao.generateInvite(
						user.id,
						params.id,
						body.expiresIn
					)
					if (result.error === "FORBIDDEN") {
						return status(403, {
							error: "You must be an admin to generate invite links.",
						})
					}
