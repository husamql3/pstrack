...
- **Email self-hosting is specified in [ADR 0013](0013-self-hosted-email.md).** Postal (named below) was rejected as too heavy for this box; a single-container **Stalwart** MTA sends **directly** from the VPS IP (no smarthost relay) over SMTP, with React Email rendered in-app and a per-type cutover canary. The "Email" section below is superseded by 0013.
