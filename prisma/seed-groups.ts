/**
 * Seed script: creates 50 dummy users + 9 groups with memberships.
 *
 * Users are inserted directly via Prisma (no auth side-effects / no emails).
 * Run via: bun run prisma/seed-groups.ts
 */
...
export async function seedGroups() {
	...
	for (let gi = 0; gi < groupsData.length; gi++) {
		...
		const adminUser = users[gi] // one dedicated admin per group
		...
		// Pick members: rotate through non-admin users starting at offset gi * 5
