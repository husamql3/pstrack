...
| Email           | Stalwart (self-hosted SMTP) + React Email           |
...
Transactional emails are sent through **self-hosted Stalwart** over SMTP (`nodemailer`) with **React Email** templates rendered to HTML/text in-app. Templates live in `src/emails/`. All sends go through `sendEmail({ ..., tag })` in `src/server/lib/email.ts`, which routes by `EMAIL_TRANSPORT` (`smtp` | `resend` | `log`; `log` in dev/staging) and a per-`tag` canary override (`EMAIL_RESEND_TAGS`) while Resend is being retired. See [ADR 0013](docs/adr/0013-self-hosted-email.md).
